...
name_vendor_url="https://github.com/omonomo/Ubroit"
name_designer=""
name_designer_url=""
...
# J (リーゼントを少しカット)
    ...
    PasteWithOffset(0, -450)
    PasteWithOffset(0, 810)
    RemoveOverlap()
    Copy()
    Select(0u004a) # J
    SelectMore(0u0134) # Ĵ
...
# スラッシュ0を作成
    # 通常 (上付き、下付きは後で加工)
    Select(${address_store_zero}); Copy() # 保管したドット無し0
    ...
    RemoveOverlap()
    Copy()
    Select(${address_store_zero} + 6) # 保管所
    PasteWithOffset(0, 45)
    RemoveOverlap()
    SetWidth(${width_latin})
    Copy()
...
# < (ss18 用のグリフを作る)
    ...
    if (input_list[i] == "${input_latin_regular}")
        Scale(150, 50, 261 + ${move_y_math})
    else
        Scale(145, 46, 263 + ${move_y_math})
    endif
    Copy()
    ...
    Select(${address_store_ssliga}) # 保管所
    if (input_list[i] == "${input_latin_regular}")
        PasteWithOffset(-120 - 0, 0)
    else
        PasteWithOffset(-130 - 0, 0)
    endif
    OverlapIntersect()
...
# > (ss18 用のグリフを作る)
    ...
    Select(65575); Paste() # Temporary glyph
    if (input_list[i] == "${input_latin_regular}")
        Scale(150, 469, 261 + ${move_y_math})
    else
        Scale(145, 470, 262 + ${move_y_math})
    endif
    Copy()
