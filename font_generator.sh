...
# 上付き、下付き用
move_y_super="282" # 基本から作成した上付き文字のY座標移動量
move_y_super2="0" # 上付きからのY座標移動量
move_y_sub="0" # 下付きY座標移動量
scale_width_super_sub="64" # グリフX座標の拡大率
scale_height_super_sub="54" # グリフY座標の拡大率
weight_super_sub="0" # ウェイト調整量
scale_super_sub2="134" # 上付き、下付きからの拡大率
...
# Loose 版用
scale_width_latin_loose="104" # Latin フォントの半角英数文字の横拡大率 (Loose 版)
...
# i (右に移動)
 ...
 #    SelectMore(0u1ec9) # ỉ
    ...
    SelectMore(0u1f76, 0u1f77) # ὶί
    SelectMore(0u1fd0, 0u1fd3) # ῐ-ΐ
...
# ドット無し0を作成
    ...
    Scale(${scale_width_super_sub}, ${scale_height_super_sub}, 256, 0)
    Move(0, ${move_y_super})
    Scale(${scale_super_sub2}, 256, ${move_y_super} + 180)
    SetWidth(${width_hankaku})
    ...
    Scale(${scale_width_super_sub}, ${scale_height_super_sub}, 256, 0)
    Move(0, ${move_y_sub})
    Scale(${scale_super_sub2}, 256, ${move_y_sub} + 180)
    SetWidth(${width_hankaku})
...
# ⁄ (/と区別するため分割)
    ...
    Select(0u2044) # ⁄
    if (input_list[i] == "${input_latin_regular}")
        PasteWithOffset(245, 405); PasteWithOffset(-245, -405)
    else
        PasteWithOffset(230, 405); PasteWithOffset(-230, -405)
    endif
    SetWidth(${width_hankaku})
    OverlapIntersect()
...
# Change the scale of hankaku glyphs
 ...
 #        SelectMore(0u1d538, 0u1d56b) # 数学用英数字記号
        ...
        foreach
            if (WorthOutputting())
                if (GlyphInfo("Width") <= 700)
                    Scale(${scale_width_latin}, ${scale_width_latin}, 256, ${center_height_hankaku})
                    Move(${move_x_hankaku_latin}, 0)
    	              SetWidth(${width_hankaku})
                endif
            endif
...
# 半角英数記号を全角形にコピー、加工
 ...
 #    SetWidth(${width_zenkaku})
    ...
    Move(76, 0)
    Select(0u003F); Copy() # ?
    Select(0u2049); PasteWithOffset(461, 0) # ⁉
    Move(-${move_x_hankaku}, 0)
    SetWidth(${width_zenkaku})
